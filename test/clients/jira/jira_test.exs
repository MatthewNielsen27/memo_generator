defmodule MemoGenerator.Clients.JiraTest do
        ...
        "comments" => [
          %{
            "author" => %{
              ...
              "avatarUrls" => %{
                "48x48" =>
                  "https://jira.lblw.cloud/secure/useravatar?ownerId=yuxuan.hu&avatarId=25419"
              }
            },
            "body" =>
              "I believe is the query that jeanie use is outdated. Need to confirm the query in the request",
  ...
  setup do
    ...
    MemoGenerator.Clients.Config.update(Jira, %{
      "headers" => [],
  ...
  test "sort_by/2" do
    assert MemoGenerator.Clients.Jira.sort_by([@clean_issue], "priority") == %{
             "P1" => [@clean_issue]
           }

    assert MemoGenerator.Clients.Jira.sort_by([@clean_issue], "project") == %{
             "A" => [@clean_issue]
           }
  end
  ...
  test "get_board_data/2" do
    assert MemoGenerator.Clients.Jira.get_board_data("1", "priority") == %{
             "name" => "BOARD A",
             "type" => "Kanban",
             "issues" => %{"P1" => [@clean_issue]}
           }
  end
  ...
  test "client/0" do
    assert MemoGenerator.Clients.Jira.client() == %Tesla.Client{
             ...
             pre: [
               ...
               {Tesla.Middleware.BaseUrl, :call, ["https://jira.example.com/rest/api/2"]}
             ]
           }
  end
end
